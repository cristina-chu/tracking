...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None: 
            ...
            for pos in self.legalPositions:
                distance = util.manhattanDistance(pos, pacmanPosition)
                allPossible[pos] += emissionModel[distance] * belief[pos]
            
            #all particles receive 0 weight
            if not any(allPossible.values()):
                ...
                for i in range(self.numParticles):
                    new.append(util.sample(allPossible)) #generate a sample from belief distribution
